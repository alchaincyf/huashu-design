...
if (check) {
  const staged = fs.mkdtempSync(path.join(os.tmpdir(), 'huashu-design-plugin-leaf-'));
  try {
    const expected = path.join(staged, 'huashu-design');
    materialize(expected);
    if (!fs.existsSync(leafRoot)) {
      throw new Error('plugins/huashu-design/skills/huashu-design is missing — run node scripts/build-plugin-leaf.mjs');
    }
