...
Detect the language of the user's most recent input. Conversational replies, placeholder text, slide copy, button labels, microcopy, and any other natural-language content in generated artifacts must match that language. Code, file names, CSS class names, variable names, and console messages stay in English regardless.

Currently supported triggers: English, Portuguese (BR/PT), Urdu. For Urdu output, no dedicated RTL reference is wired up yet — use `dir="rtl"` on the root element and pick a Nastaliq web font (Noto Nastaliq Urdu via Google Fonts is a safe default), but treat this as best-effort until a dedicated RTL reference is added.

If the user mixes languages or asks explicitly for a different output language, follow the explicit request.
