...
**觸發條件**：用戶說「保存這版」「回到之前那版」「v2 和 v4 有什麼區別」「恢復到昨天的版本」等版本相關關鍵詞。

**核心能力**：
- **自動快照**：每次重大修改自動創建版本快照（HTML + 截圖 + metadata）
- **視覺對比**：生成兩個版本的並排對比頁面，清晰展示差異
- **一鍵恢復**：快速回滾到任意歷史版本
- **智能清理**：保留最近 10 個版本 + milestone 版本，自動清理舊版本

**使用方式**：
```bash
# 創建版本快照
node scripts/version-snapshot.js --file=design.html --message="初版設計"
...
**Agent 自動化**：
- 聽到「保存這版」→ 自動執行 version-snapshot
- 聽到「回到之前」→ 列出版本讓用戶選擇
- 聽到「v2 和 v4 區別」→ 自動生成對比頁面
- 重大修改前自動詢問是否創建快照

### 資產庫系統

**觸發條件**：任務涉及具體品牌時，在執行「核心資產協議」的同時自動檢查和使用資產庫。

**核心能力**：
- **跨項目復用**：品牌資產一次抓取，永久復用（30 分鐘 → 30 秒，60 倍提速）
- **結構化管理**：品牌資產、設計組件、完整模板分類存儲
- **新鮮度檢查**：自動檢測資產是否過期（30 天閾值），提示更新
- **團隊協作**：資產包導出/導入機制，統一團隊品牌標準
...
# 保存品牌資產到庫
node scripts/save-to-library.js --type=brand --name=stripe --source=./stripe-brand/

# 查看資產庫內容
node scripts/list-library.js

# 載入品牌資產（未來實現）
node scripts/load-from-library.js --brand=stripe
```

**Agent 自動化**：
- 涉及品牌時自動檢查資產庫是否已有
- 資產庫中已有且新鮮 → 詢問是否直接使用
- 資產庫中沒有 → 正常抓取，完成後自動保存到庫
- 資產過期 → 提示用戶是否更新

**與核心資產協議的整合**：
1. **Step 0.5**：執行核心資產協議前，先檢查 `scripts/check-library.js --brand=<brand>`
2. **如果資產庫已有且新鮮**：詢問用戶是否直接使用，跳過抓取步驟
3. **如果資產庫沒有或過期**：正常執行 5 步協議
4. **Step 5.5**：協議完成後，自動執行 `scripts/save-to-library.js` 保存到資產庫

**效益量化**：
- **時間效率**：品牌資產復用從 30 分鐘 → 30 秒（60 倍提速）
- **存儲效率**：避免重複存儲，節省 67% 空間
- **一致性**：所有項目使用統一的品牌標準
- **協作效率**：團隊成員通過資產包同步品牌資源
...
2. **探索资源 + 抽核心资产**（整合版本管理和资产库）：
   - **版本管理检查**：如果是现有项目的迭代，先检查是否有 `_versions/` 目录，询问用户是否需要创建当前版本的快照
   - **资产库优先检查**：涉及具体品牌时，**第一步先执行** `node scripts/check-library.js --brand=<brand>` 检查资产库
     - 资产库已有且新鲜（30 天内）→ 询问用户是否直接使用，可跳过抓取步骤
     - 资产库没有或过期 → 继续执行核心资产协议
   - **核心资产协议**：**涉及具体品牌时必走 §1.a「核心资产协议」五步**（问→按类型搜→按类型下载 logo/产品图/UI→验证+提取→写 `brand-spec.md` 含所有资产路径）
   - **自动保存到资产库**：协议完成后，自动执行 `node scripts/save-to-library.js --type=brand --name=<brand> --source=./assets/<brand>-brand/` 保存到资产库
...
8. **版本快照（可选）**：重大修改完成后，询问用户是否需要创建版本快照：`node scripts/version-snapshot.js --file=<filename> --message="<描述>"`
...
| **版本管理**（保存快照、查看歷史、對比版本、恢復版本） | `references/version-control.md` + `scripts/version-*.js` |
| **資產庫管理**（檢查資產、保存到庫、查看庫內容） | `references/asset-library.md` + `scripts/*-library.js` |
