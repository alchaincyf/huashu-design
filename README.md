...
| **版本管理** | **自动快照 · 视觉对比 · 一键恢复 · 智能清理** | **2–5 min** |
| **资产库** | **跨项目复用 · 60倍提速 · 团队协作 · 新鲜度检查** | **30秒 vs 30分钟** |
...
自动快照每次重大修改，生成视觉对比页面，一键恢复任意版本。

**核心能力**：
- **自动快照**：HTML + 截图 + metadata，智能清理策略
- **视觉对比**：并排展示两版本差异，清晰标注变更
- **一键恢复**：快速回滚到任意历史版本
- **Agent 自动化**：听到「保存这版」「回到之前」自动触发
...
品牌资产一次抓取，永久复用。30 分钟变 30 秒，60 倍提速。

**核心能力**：
- **跨项目复用**：品牌资产全局共享，避免重复抓取
- **结构化管理**：品牌/组件/模板分类存储，快速检索
- **新鲜度检查**：30 天阈值，自动提示资产更新
- **团队协作**：资产包导出/导入，统一品牌标准

**存储效率**：
- 3 个 Stripe 项目：150MB → 50MB（节省 67%）
- 品牌资产复用：30 分钟 → 30 秒（60 倍提速）
- 隐私保护：敏感资产不会意外分享到版本控制
...
│   ├── version-control.md   # 版本管理详细文档
│   ├── asset-library.md     # 资产库详细文档
...
│   ├── video-export.md
│   └── ...
├── scripts/                 # 导出工具链 + 版本管理 + 资产库
│   ├── render-video.js      # HTML → MP4
│   ├── convert-formats.sh   # MP4 → 60fps + GIF
│   ├── add-music.sh         # MP4 + BGM
│   ├── version-snapshot.js  # 创建版本快照
│   ├── version-list.js      # 查看版本历史
│   ├── version-diff.js      # 版本对比
│   ├── version-restore.js   # 恢复版本
│   ├── save-to-library.js   # 保存到资产库
│   ├── list-library.js      # 查看资产库
│   ├── check-library.js     # 检查资产库
│   ├── export_deck_pdf.mjs
